...
function get_branch_beta_version {
    echo $(echo $(git describe --abbrev=0) | sed -e 's,disman@\(\),\1,g')
}
...
# Something like: disman@0.5XX.0-beta.X
...
standard-version -t disman\@ --skip.commit true --skip.tag true --preMajor --release-as $NEXT_BETA_VERSION
...
git tag -a "disman@${NEXT_BETA_VERSION}" -m "Create beta release ${NEXT_BETA_VERSION}."
